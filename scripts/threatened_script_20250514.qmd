...
calculate_prop_traits_scored_scaled <- function(data, trait_table) {
  ...
  trait_weights <- trait_table %>%
    dplyr::mutate(effect_score = abs(effect_score)) %>%
    dplyr::select(-notes) %>%
  ...
  data %>%
    rowwise() %>%
    mutate(
      weighted_traits_scored = sum(
        sapply(traits, function(trait) {
          if (!is.na(cur_data()[[trait]])) trait_weights[[trait]] else 0
        })
      ) / total_effect_scores
    ) %>%
    ungroup()
...
calculate_threat_risk <- function(
  ...
  data <- data %>%
    calculate_prop_traits_scored(trait_table = trait_effects) %>%
    mutate(
...
phytophthora_assigned_risk <- phytophthora_assigned_risk  %>%
    ...
    # proportion_relevant traits scored
      calculate_prop_traits_scored(trait_table = phytophthora_trait_effects) %>%
    # alternative confidence calculation that scales scores; very slow!
...
myrtle_rust_assigned_risk <- myrtle_rust_assigned_risk %>%
    # calculate confidence in risk score, based on how much data available
    # proportion_relevant traits scored
      calculate_prop_traits_scored(trait_table = myrtle_rust_trait_effects) %>%
    # alternative confidence calculation that scales scores; very slow!
...
goats_assigned_risk <- goats_assigned_risk %>%
   # calculate confidence in risk score, based on how much data available
    # proportion_relevant traits scored
      calculate_prop_traits_scored(trait_table = goat_trait_effects) %>%
...
deer_assigned_risk <- deer_assigned_risk %>%
   ...
   # calculate confidence in risk score, based on how much data available
    # proportion_relevant traits scored
      calculate_prop_traits_scored(trait_table = deer_trait_effects) %>%
...
pigs_assigned_risk <- pigs_assigned_risk %>%
   # calculate confidence in risk score, based on how much data available
    # proportion_relevant traits scored
      calculate_prop_traits_scored(trait_table = pigs_trait_effects) %>%
...
tropical_C4_grasses_assigned_risk <- tropical_C4_grasses_assigned_risk %>%
   # calculate confidence in risk score, based on how much data available
    # proportion_relevant traits scored
      calculate_prop_traits_scored(trait_table = tropical_C4_grasses_trait_effects) %>%
...
# assign risk by category
  tropical_C4_grasses_assigned_risk <- tropical_C4_grasses_assigned_risk %>%
    mutate(
      # C4 grasses uses different bin thresholds to every other threat, hence the
      # explicit bin_thresholds argument (all other threats rely on create_risk_bins()'s default)
      total_risk_score = create_risk_bins(total_risk, bin_thresholds = c(
...
hymenachne_assigned_risk <- hymenachne_assigned_risk %>%
   ...
   # calculate confidence in risk score, based on how much data available
    # proportion_relevant traits scored
      calculate_prop_traits_scored(trait_table = hymenachne_trait_effects) %>%
...
threat_independent_risk <- threat_independent_risk_tmp %>%
   ...
   # calculate confidence in risk score, based on how much data available
    # proportion_relevant traits scored
      calculate_prop_traits_scored(trait_table = threat_independent_effects) %>%
...
precariousness_risk <- precariousness_risk_tmp %>%
  ...
  # calculate confidence in risk score, based on how much data available (no habitat term here)
    calculate_prop_traits_scored(trait_table = precariousness_effects) %>%
...
precariousness_risk %>%
  filter(!is.na(threatened_status)) %>%
  select(taxon_name, genus, family, threatened_status, total_risk_score,
         risk_confidence, total_risk, trait_effect_score_scaled,
