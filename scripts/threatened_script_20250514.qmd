...
#### calculate_treat_risk

```{r}
# Claude coded, from my original script, abstracting sections that were previously repeated. TBD how well it works
...
#                  default matches the standard 5-bin scheme used elsewhere
...
calculate_threat_risk <- function(
  ...
  for (tr in traits_to_score) {
    risk_col <- paste0(tr, "_risk")
    eff <- get_effect_score(trait_effects, tr)$effect_score
    data[[risk_col]] <- calculate_risk(tr, eff, data)
  }
  ...
  data <- data %>%
    mutate(
      ...
      # Step 7: bin
      total_risk_score = cut_risk_bins(total_risk, bin_thresholds)
    )
...
cut_risk_bins <- function(x, bin_thresholds) {
...
myrtle_rust_susceptibility_data <- myrtle_rust_susceptibility %>%
  ...
  left_join(resources$APC %>% select(taxon_name = canonical_name, genus, family) |> distinct, by = "taxon_name")
```
...
myrtle_rust_trait_effects <- tribble(
  ...
  "leaf_lignin_per_dry_mass", -0.2, "higher leaf lignin = lower impact"
)
  ...
  mutate(
    ...
    # assign trait risk effect scores; trait effect scores only apply to Myrtaceae, not other rainforest plants
      ...
      bud_bank_location_scaled_risk = ifelse(family == "Myrtaceae",
                                       calculate_risk("bud_bank_location_scaled", get_effect_score(myrtle_rust_trait_effects, "bud_bank_location_scaled")$effect_score, .), NA),
...
goats_assigned_risk <- goats_assigned_risk %>%
  ...
  mutate(
      ...
      # add trait effect scores
        ...
        reproductive_maturity_scaled_risk = calculate_risk("reproductive_maturity_scaled", get_effect_score(goat_trait_effects, "reproductive_maturity_scaled")$effect_score, .),
...
hymenachne_assigned_risk <- hymenachne_assigned_risk %>%
  ...
  mutate(
    ...
    # annuals, ephemerals have highest impact
      ...
      pollination_syndrome_risk = calculate_risk("pollination_syndrome", get_effect_score(hymenachne_trait_effects, "pollination_syndrome")$effect_score, .),
...
- this section includes species' ability to regenerate
...
threat_independent_effects <- tribble(
  ...
  #"pollination_syndrome_scales", -0.5, "more specific pollinators, requires bigger population, less overall disturbance"
)
...
threat_independent_risk_tmp <- native_species %>%
  ...
  left_join(woodiness, by = "taxon_name") %>%
  left_join(herb_shrub_tree, by = "taxon_name") %>%
  left_join(lifplant_height_scaled, by = "taxon_name")
  #left_join(dispersers, by = "taxon_name") %>%
  #left_join(seedbank_location, by = "taxon_name") %>%
  #left_join(vegetative_reproduction_ability, by = "taxon_name") %>%
  #left_join(seed_dry_mass, by = "taxon_name") %>%
  #left_join(reproductive_maturity, by = "taxon_name") %>%
  #left_join(lifespan, by = "taxon_name") %>%
...
threat_independent_risk <- threat_independent_risk_tmp %>%
  ...
  mutate(
    # assign trait risk effect scores
    # perennials more impacted
      life_history_risk = calculate_risk("life_history", get_effect_score(threat_independent_effects, "life_history")$effect_score, .),
      
    # woody species more impacted
      woodiness_risk = calculate_risk("woodiness", get_effect_score(threat_independent_effects, "woodiness")$effect_score, .),
    
    # trees, shrubs more impacted
    herb_shrub_tree_risk = calculate_risk("herb_shrub_tree", get_effect_score(threat_independent_effects, "herb_shrub_tree")$effect_score, .),
    
    # tall plants more impacted
      plant_height_scaled_risk = calculate_risk("plant_height_scaled", get_effect_score(threat_independent_effects, "plant_height_scaled")$effect_score, .),
    
    # # smaller seeds = lower impact
    #   seed_dry_mass_scaled_risk = calculate_risk("seed_dry_mass_scaled", get_effect_score(threat_independent_effects, "seed_dry_mass_scaled")$effect_score, .),
    # 
    # # better dispersal = lower risk
    #   dispersers_risk = calculate_risk("dispersers", get_effect_score(threat_independent_effects, "dispersers")$effect_score, .),
    # 
    # # seedbank present = lower risk
    #   seedbank_location_risk = calculate_risk("seedbank_location", get_effect_score(threat_independent_effects, "seedbank_location")$effect_score, .),
    # 
    # # faster reproductive maturity = lower risk
    #   reproductive_maturity_scaled_risk = calculate_risk("reproductive_maturity_scaled", get_effect_score(threat_independent_effects, "reproductive_maturity_scaled")$effect_score, .),   
    # 
    # # shorter lifespan = lower risk
    #   lifespan_scaled_risk = calculate_risk("lifespan_scaled", get_effect_score(threat_independent_effects, "lifespan_scaled")$effect_score, .),  
    # 
    # # vegetative reproduction ability = lower risk
    #   vegetative_reproduction_ability_risk = calculate_risk("vegetative_reproduction_ability", get_effect_score(threat_independent_effects, "vegetative_reproduction_ability")$effect_score, .),
    # # EOO = lower risk
    #   EOO_scaled_risk = calculate_risk("extent_of_occurrence_sigmoid_scaled", get_effect_score(threat_independent_effects, "extent_of_occurrence_sigmoid_scaled")$effect_score, .),
    

    # sum trait risk effect scores
      total_trait_effect_score = rowSums(across(c(seed_dry_mass_scaled_risk, seedbank_location_risk, vegetative_reproduction_ability_risk, dispersers_risk, lifespan_scaled_risk, reproductive_maturity_scaled_risk, EOO_scaled_risk)), na.rm = TRUE)
  ) %>%
...
precariousness_risk <- precariousness_risk_tmp %>%
  ...
  mutate(
    ...
    # smaller range = higher risk
      EOO_scaled_risk = calculate_risk("extent_of_occurrence_sigmoid_scaled", get_effect_score(precariousness_effects, "extent_of_occurrence_sigmoid_scaled")$effect_score, .),

      AOO_scaled_risk = calculate_risk("area_of_occupancy_sigmoid_scaled", get_effect_score(precariousness_effects, "area_of_occupancy_sigmoid_scaled")$effect_score, .),

    # sum trait risk effect scores
      total_trait_effect_score = rowSums(across(c(EOO_scaled_risk, AOO_scaled_risk)), na.rm = TRUE)
  ) %>%
...
### Comparisons with non-threatened species

Do threatened species have worse dispersal, longer lifespans, etc?

- Simple answer is NO. There is minimal difference in any of the traits measured between threatened vs other species, except that reproductive maturity is higher for threatened taxa
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
threat_independent_risk_tmp <- threat_independent_risk_tmp %>%
  mutate(threatened = ifelse(!is.na(threatened_status), "threatened", "other"))

correlations_with_possible_traits <- tibble(
  trait = NA_character_,
  other_mean = NA_character_,
  other_SE = NA_character_,
  threatened_mean = NA_character_,
  threatened_SE = NA_character_,
  r2 = NA_character_, 
  p_value = NA_character_,
  n = NA_character_,
)

for (var in c("seed_dry_mass_log10", "reproductive_maturity_log10", "seedbank_location", "life_history", "dispersers", "vegetative_reproduction_ability", "lifespan_log10")) {
  
  data_tmp <- threat_independent_risk_tmp %>% filter(!is.na(threat_independent_risk_tmp[[var]]))
  
  linearmodel_var <- lm(data_tmp[[var]] ~ threatened , data = data_tmp)
  
  
  linearmodel_coeff <- summary(linearmodel_var)$coefficients %>% as.data.frame() %>% slice(2)
  
  estimate_means_output <- modelbased::estimate_means(linearmodel_var)  %>% as.data.frame()
  estimate_means_output[1,2]
  
  new_row <- c(
    trait = var, 
    r2= round(summary(linearmodel_var)$r.squared,4), 
    p_value = round(linearmodel_coeff[1,4],4),
    other_mean = round(estimate_means_output[1,2],3),
    threatened_mean = round(estimate_means_output[2,2],3),
    other_SE = round(estimate_means_output[1,3],3),
    threatened_SE = round(estimate_means_output[2,3],3),
    n = nrow(data_tmp)
      ) %>% 
    t() %>%
    as.data.frame()
  ...
  plot(
    ggplot(data = data_tmp, aes(x = threatened, y = data_tmp[[var]])) +
