...
source("R/custom_R_code.R") # not required if you read in complete RDS file
...
#all_traits <-readRDS("~/GitHub/threatened_species_traits/ignore/austraits/all_traits_20250620.rds") # static file with all data 2025-06-16
```
...
vegetative_reproduction_ability_scores <- read_csv("data_extras/vegetative_reproduction_ability_scores.csv")
seedbank_location_scores <- read_csv("data_extras/seedbank_location_scores.csv")
...
pollination_syndrome_scores <- read_csv("data_extras/pollination_syndrome_scores.csv")
...
phytophthora_assigned_risk <- phytophthora_assigned_risk  %>%
  ...
  phytophthora_assigned_risk %>%
    ...
    write_csv("export/phytophthora_assigned_risk_scores.csv", na = "")
...
threat_independent_effects <- tribble(
  ~trait,                  ~effect_score, ~notes,
  "life_history",            0.3,          "annuals, ephemerals = faster regeneration",
  ...
  "extent_of_occurrence_sigmoid_scaled", -0.3, "bigger range, regenerates faster",
  "area_of_occupancy_scaled", -0.3, "bigger range, regenerates faster"
)
...
threat_independent_risk <- threat_independent_risk_tmp %>%
  ...
  mutate(
    # assign trait risk effect scores
    # perennials have highest impact
  ...
  # REQUIRED BECAUSE SO MANY TAXA HAVE NO DATA FOR ANY TRAITS EXCEPT LIFE HISTORY
...
# assign risk by category
  threat_independent_risk %>%
    ...
    select(taxon_name, genus, family, threatened_status, total_risk, 
           ...
           prop_traits_scored, 
           contains(c("risk_effect_score")), everything()) %>%
    write_csv("export/threat_independent_risk_scores.csv", na = "")
```
