...
```{r}
# Claude coded, from my original script, abstracting sections that were previously repeated.

# ---- Generalised risk calculation function ----
# data: dataframe already joined with all relevant traits/habitat/taxonomy columns
# trait_effects: tribble with columns trait, effect_score, notes (as before)
# habitat_col: name (string) of the habitat effect score column (e.g. "goat_habitat_effect_score")
# habitat_default: value to use when habitat_col is NA (default 0.5)
# rescale_range: numeric vector c(min, max) for scales::rescale - the biologist-chosen calibration
# confidence_low: multiplier for prop_traits_scored when habitat unknown (default 0.15)
# confidence_high: multiplier for prop_traits_scored when habitat known (default 0.6)
# bin_thresholds: named numeric vector of lower bounds for each bin, descending.
#                  NULL (default) defers to create_risk_bins()'s own default, so the
#                  standard 5-bin scheme is defined in exactly one place - see there.
# extra_risk_terms: optional list of additional risk columns (already computed upstream,
#                    e.g. taxonomic_risk, forb_risk) to include in the sum

calculate_threat_risk <- function(
    data,
    trait_effects,
    habitat_col,
    rescale_range,
    ...
    confidence_high = 0.6,
    bin_thresholds = NULL,
    extra_risk_terms = character(0)
) {

  habitat_col_sym <- sym(habitat_col)

  # Step 1-2: confidence + habitat risk
  data <- data %>%
    calculate_prop_traits_scored_scaled(trait_table = trait_effects) %>%
    mutate(
      risk_confidence = ifelse(
        is.na(!!habitat_col_sym),
        confidence_low * prop_traits_scored,
        confidence_high * prop_traits_scored
      ),
      habitat_risk = ifelse(is.na(!!habitat_col_sym), habitat_default, !!habitat_col_sym)
    )

  # Step 3: compute {trait}_risk for every trait in trait_effects
  # (skips traits with effect_score = NA, e.g. ones handled separately/manually)
  data <- data %>% add_trait_risks(trait_effects)
  traits_to_score <- trait_effects %>% filter(!is.na(effect_score)) %>% pull(trait)

  # Step 4: sum trait risks + any extra pre-computed terms (e.g. taxonomic_risk)
  risk_cols <- c(paste0(traits_to_score, "_risk"), extra_risk_terms)
  risk_cols <- risk_cols[risk_cols %in% names(data)]  # guard against typos/missing cols

  data <- data %>%
    mutate(
      total_trait_effect_score = rowSums(across(all_of(risk_cols)), na.rm = TRUE),

      # Step 5: rescale (biologist-calibrated range)
      trait_effect_score_scaled = scales::rescale(total_trait_effect_score, to = rescale_range),

      # Step 6: combine with habitat
      total_risk = habitat_risk * trait_effect_score_scaled,

      # Step 7: bin (NULL bin_thresholds falls through to create_risk_bins()'s own default)
      total_risk_score = if (is.null(bin_thresholds)) create_risk_bins(total_risk) else create_risk_bins(total_risk, bin_thresholds)
    )

  data
}

# helper: turn total_risk into ordered categorical bins from a named threshold vector.
# Replaces the repeated 5-line `total_risk_score = case_when(total_risk >= 0.8 ~ "5_extremely_high", ...)`
# block used in every threat section. `bin_thresholds` defaults to the standard scheme
# used by most threats (0.8/0.6/0.4/0.2); pass a different named vector for threats
# that use different cutoffs (e.g. C4 grasses uses 0.7/0.4/0.3/0.15). This is the ONLY
# place the standard scheme's values should be defined - calculate_threat_risk() defers
# to this default rather than keeping its own copy, so there's a single source of truth.
...
# Formalises turning risk_confidence into a plausible range around a numeric risk
# score, instead of the point estimate alone. Low confidence -> wide range; high
# confidence -> narrow range.
#
...
#
# divisor defaults to 5 (i.e. at confidence 0, margin is 1/5 = 0.2 either side, rather
# than the full 0-1 range; at confidence 0.6 - roughly the top of the observed range -
# margin is 0.4/5 = 0.08) - change it here (or per-call) and every threat regenerates.
#
# risk_col/confidence_col: names (strings) of the numeric risk and confidence columns
# to use as inputs. output_base: name to build the four new columns from - defaults to
# risk_col, but can differ, e.g. for Phytophthora/myrtle rust, where the numeric value
# that actually determines total_risk_score is total_risk_overwrite (not total_risk),
# so risk_col = "total_risk_overwrite" but output_base = "total_risk" keeps the output
# column names (total_risk_min/max/score_min/score_max) consistent with every other threat.
# bin_thresholds: passed through to create_risk_bins(); NULL uses its own default.
...
for (var in c("seed_dry_mass", "leaf_mass_per_area", "leaf_N_per_dry_mass", "leaf_P_per_dry_mass")) {
  ...
  linearmodel_var <- lm(phytophthora_cinnamomi_susceptibility ~ log10(as.numeric(data_tmp[[var]])), data = data_tmp)
  ...
  if(new_row[1,]$r2 > 0.03 & new_row[1,]$p_value < 0.05) {
    ...
    plot(
           ggplot(data_tmp, aes(x = log10(as.numeric(data_tmp[[var]])), y = phytophthora_cinnamomi_susceptibility)) +
            geom_point() +
            geom_jitter() +
            geom_smooth(method = "lm", se = FALSE, color = "grey20") +
            theme_classic() +
...
for (var in c("seed_dry_mass", "leaf_mass_per_area", "plant_height", "leaf_length", "leaf_N_per_dry_mass", "leaf_P_per_dry_mass", "leaf_lignin_per_dry_mass", "leaf_phenol_per_dry_mass", "leaf_tannin_per_dry_mass")) {
  ...
  linearmodel_var <- lm(myrtle_rust_susceptibility ~ log10(as.numeric(data_tmp[[var]])), data = data_tmp)
  ...
  new_row <- c(
    ...
    if(new_row[1,]$r2 > 0.04 & new_row[1,]$p_value < 0.05) {
      ...
      plot(
             ggplot(data_tmp, aes(x = log10(as.numeric(data_tmp[[var]])), y = myrtle_rust_susceptibility)) +
              geom_point() +
              geom_jitter() +
              geom_smooth(method = "lm", se = FALSE, color = "grey20") +
              theme_classic() +
...
for (var in c("leaf_mass_per_area", "plant_height", "leaf_N_per_dry_mass", "leaf_P_per_dry_mass", "digestible_mass_per_dry_mass", "leaf_crude_protein_per_dry_mass", "metabolisable_energy", "leaf_work_to_shear_adjusted")) {
  ...
  linearmodel_var <- lm(plant_goat_palatability ~ log10(as.numeric(data_tmp[[var]])), data = data_tmp)
  ...
  new_row <- c(
    ...
    as.data.frame()
      ...
      plot(
             ggplot(data_tmp, aes(x = log10(as.numeric(data_tmp[[var]])), y = plant_goat_palatability)) +
              geom_point() +
              geom_jitter() +
              geom_smooth(method = "lm", se = FALSE, color = "grey20") +
              theme_classic() +
...
goats_assigned_risk <- goats_assigned_risk %>%
  ...
  mutate(
    ...
    # maturity, woodiness - NB woodiness_risk is computed here but, as in the original
    # code, not included in the rowSums below; flagging in case that's an oversight)
    add_trait_risks(goat_trait_effects) %>%

    mutate(
      # sum together trait and taxonomic effect scores
        total_trait_effect_score = rowSums(across(c(plant_height_4m_peak_scaled_risk, resprouting_capacity_risk, bud_bank_location_scaled_risk, forb_risk, life_history_risk, bark_thickness_scaled_risk, taxonomic_risk, reproductive_maturity_scaled_risk)), na.rm = TRUE),
...
pigs_assigned_risk <- pigs_assigned_risk %>%
   # calculate confidence in risk score, based on how much data available, weighted
    # by trait effect size. (Comparison against the unweighted trait-count version
    # checked out - see phytophthora - so commented out below rather than deleted). NB
    # pigs_trait_effects has one NA-effect_score trait (fruit_fleshiness), treated as
    # weight 0 in the scaled version, so it's counted in neither numerator nor denominator
  ...
  mutate(
    ...
    # resprouting capacity, storage organ, fruit length - renamed from the old
    # "fruit_risk" to "fruit_length_scaled_risk" to match every other threat's naming
    # convention; NB this changes the column name in the exported CSV)
