...
- you can source the most recent austraits.build (AusTraits) release from Zenodo (soon to be 6.1) or use a data file provided
...
phytophthora_assigned_risk <- phytophthora_assigned_risk  %>%
  ...
  phytophthora_assigned_risk <- phytophthora_assigned_risk %>%
    ...
    # risk_confidence_trait_count = ifelse(!is.na(phytophthora_cinnamomi_susceptibility), 0.8, risk_confidence_trait_count),
      risk_confidence = ifelse(!is.na(phytophthora_cinnamomi_susceptibility), 0.8, risk_confidence),
  ...
  phytophthora_assigned_risk %>%
    ...
    select(taxon_name, genus, family, threatened_status, total_risk_score,
           phytophthora_cinnamomi_susceptibility, risk_confidence,
...
myrtle_rust_assigned_risk <- myrtle_rust_assigned_risk %>%
  ...
  mutate(
    ...
    # add {trait}_risk for every trait in myrtle_rust_trait_effects (plant height, leaf
    # length, resprouting capacity, leaf lignin, bud bank location); trait effect scores
    # only apply to Myrtaceae, not other rainforest plants, hence the condition
...
deer_assigned_risk <- deer_assigned_risk %>%
  ...
  mutate(
      ...
      # NB reproductive_maturity_scaled is commented out of deer_trait_effects below, so
      # it can't go through add_trait_risks (which only loops over active tribble rows) -
      # kept as its own manual line, unchanged from before, borrowing life_history's
      # effect score (both were -0.2, so this matches what a live reproductive_maturity_scaled
      # row would have given anyway - see the commented-out line in the tribble)
...
tropical_C4_grasses_assigned_risk <- tropical_C4_grasses_assigned_risk %>%
  ...
  mutate(
    ...
    # dry_season_flowering_risk is computed here but not used below - the raw
    # dry_season_flowering trait value is instead used directly, with two different
...
hymenachne_assigned_risk <- hymenachne_assigned_risk %>%
  ...
  mutate(
    ...
    # add {trait}_risk for every remaining trait in hymenachne_trait_effects (plant
    # height, woodiness, life history, pollination syndrome). plant_growth_substrate_hymenanche
...
precariousness_risk <- precariousness_risk_tmp %>%
  ...
  # REQUIRED - taxa with neither EOO nor AOO data would otherwise show a risk of 0 (= minimum risk, rather than "unknown")
