...
tropical_C4_grasses_assigned_risk <- tropical_C4_grasses_assigned_risk %>%
  ...
  mutate(
    ...
    # resprouting capacity, bud bank location, dry season flowering). NB
    # 24-SPECIAL CASE dry_season_flowering_risk is computed here but not used below - the
    # raw dry_season_flowering trait value is instead used directly, with two different
    # hardcoded coefficients (-0.3 and -0.1) in direct_competition_risk/fire_season_risk;
    # flagging that inconsistency rather than resolving it here.
    ...
    # 27-SPECIAL CASE direct_competition_risk: hardcoded -0.3 coefficient applied directly
    # to the raw dry_season_flowering value (rather than via a trait_effects tribble entry) -
    ...
    # 28-SPECIAL CASE fire_season_risk: hardcoded -0.1 coefficient applied directly to the
    # raw dry_season_flowering value (same pattern/inconsistency as direct_competition_risk
    # above, but with a different hardcoded coefficient: -0.1 vs -0.3) - less impacted by
    # fire season if you are an annual that flowers and fruits in the late dry season, likely post-fire
...
hymenachne_trait_effects <- tribble(
  ...
  "plant_growth_substrate_hymenanche", 1, "aquatic plant at high risk",
  "pollination_syndrome", -0.3, "wind pollinated species more persistent"
)

# 29-SPECIAL CASE hymenachne_taxonomic_vector: 14 hand-picked families assumed more
# at-risk from hymenachne invasion (largely aquatic/wetland-associated families),
# all flatly assigned effect_score = 0.4 regardless of species
...
hymenachne_assigned_risk <- hymenachne_assigned_risk %>%
  ...
  mutate(
    ...
    # 30-SPECIAL CASE plant_growth_substrate_hymenanche_risk: a manual 0/1 flag (not the
    # usual effect_score * (value - mean(value)) formula) - aquatic taxa have
    # plant_growth_substrate_risk = 1
    ...
    # 31-SPECIAL CASE add {trait}_risk for every remaining trait in hymenachne_trait_effects
    # (plant height, woodiness, life history, pollination syndrome). plant_growth_substrate_hymenanche
    # is excluded here - it's already handled above by the manual aquatic-taxa rule
    # (a straight 0/1 flag, not the usual effect_score * (value - mean(value)) formula),
    # so running it through add_trait_risks too would silently overwrite that with a
    # different number under the same column name.
    add_trait_risks(hymenachne_trait_effects %>% filter(trait != "plant_growth_substrate_hymenanche")) %>%
...
precariousness_risk <- precariousness_risk_tmp %>%
  ...
  # area of occupancy) - renamed from the old "EOO_scaled_risk"/"AOO_scaled_risk" to
  # extent_of_occurrence_sigmoid_scaled_risk/area_of_occupancy_sigmoid_scaled_risk to
  # match every other threat's naming convention; NB this changes the column names in
  # the exported CSV
  ...
  # 32-SPECIAL CASE REQUIRED filter - taxa with neither EOO nor AOO data would otherwise show a risk of 0 (= minimum risk, rather than "unknown")
