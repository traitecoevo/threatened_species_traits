...
data <- read_csv("datasets_from_manuscripts/wikidata_all.csv", guess_max = 100000) %>%
  select(contains(c("taxon_name", "description", "Description", "habitat", "Habitat", "ecology", "Ecology")))

data_habitat <- data %>%
  select(contains(c("taxon_name", "habitat", "Habitat"))) %>%
  tidyr::pivot_longer(cols = 2:47) %>%
  filter(!is.na(value)) %>%
  group_by(taxon_name) %>%
  mutate(
    habitat_merged = paste0(value, collapse = "; ")
    ) %>%
  ungroup() %>%
  distinct(taxon_name, habitat_merged)

data_ecology <- data %>%
  select(contains(c("taxon_name", "ecology", "Ecology"))) %>%
  tidyr::pivot_longer(cols = 2:36) %>%
  filter(!is.na(value)) %>%
  group_by(taxon_name) %>%
  mutate(
    ecology_merged = paste0(value, collapse = "; ")
  ) %>%
  ungroup() %>%
  distinct(taxon_name, ecology_merged)

data_description <- data %>%
  select(contains(c("taxon_name", "description", "Description"))) %>%
  tidyr::pivot_longer(cols = 2:31) %>%
  filter(!is.na(value)) %>%
  group_by(taxon_name) %>%
  mutate(
    description_merged = paste0(value, collapse = "; ")
  ) %>%
  ungroup() %>%
  distinct(taxon_name, description_merged)

data <- data_habitat %>%
  rename(text = habitat_merged) %>%
...
# write to file
#write_csv(new, "datasets_from_manuscripts/Wikipedia_habitats.csv", na = "")


new %>% filter(!is.na(`05_habitat_a`)) %>% select(taxon_name, `05_habitat_a`) -> habitats

native_species %>%
  filter(!taxon_name %in% goats_habitat$taxon_name) %>%
  filter(taxon_name %in% habitats$taxon_name)
