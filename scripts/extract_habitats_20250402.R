...
data <- read_csv("data/NHNSW_2025/raw/plantNET.csv") %>%
  select(-text) %>%
  rename(text = distribution_notes) %>%
  select(taxon_name, text) %>%
  mutate(original_text = text,
         text = stringr::str_replace_all(text, "Distribution and occurrence: ", ""),
         text = stringr::str_replace_all(text, "watershed[s]", ""),
         text = stringr::str_replace_all(text, "[:punct:]", ""),
         text = stringr::str_replace_all(text, "less common", "lesscommon"),
         text = stringr::str_replace_all(text, "Not ", "Not"),         
         ...
         text = stringr::str_trim(text),
         text = stringr::str_to_lower(text),
         text = stringr::str_replace_all(text, "nsw subdivisions.*", " "),
         text = stringr::str_replace_all(text, "$", " "),
         text = stringr::str_replace_all(text, "^", " "),
         text = ifelse(text=="", NA, text)
  ) %>%
...
data <- read_csv("data/WAH_2025/raw/split_sentences_Florabase_short.csv") %>%
  rename(text = substrate_location) %>%
  select(taxon_name, text) %>%
  mutate(original_text = text,
         text = stringr::str_replace_all(text, "watershed[s]", ""),
         text = stringr::str_replace_all(text, "[:punct:]", ""),
