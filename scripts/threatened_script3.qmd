...
myrtle_rust_assigned_risk <- myrtle_rust_assigned_risk %>%
  mutate(
    taxonomic_risk = ifelse(family == "Myrtaceae", 1, 0),
    ## XX - TODO - need to merge in list of epiphytic orchids that depend on Myrtaceae
    ...
    resprouting_capacity_risk_nudge = ifelse(!is.na(resprouting_capacity),
                               -0.05*resprouting_capacity, 0),
    bud_bank_location_risk_nudge = ifelse(!is.na(bud_bank_location),
                               -0.03*bud_bank_location, 0),
    ) %>% View()
                                    
                                    correlations_with_possible_traits_myrtle_rust$coeff[match("plant_height", correlations_with_possible_traits_myrtle_rust$trait)]))) %>% View()
    
    
    ,
      height_risk_nudge = ifelse(!is.na(height_risk_nudge) & height_risk_nudge < 0, 0, -height_risk_nudge),
    resprouting_capacity_risk_nudge = 0,
    bud_bank_location_risk_nudge = 0,
    ...
    # merge risks
    myrtle_rust_risk_total = ifelse(taxonomic_risk = 0, 0, habitat_risk),
    myrtle_rust_risk_total = myrtle_rust_risk_total - trait_risk_nudge,       goat_susceptibility = case_when(
        myrtle_rust_risk_total >= 0.67 ~ "high",
        0.67 > myrtle_rust_risk_total >= 0.33 ~ "medium",
        0.33 > myrtle_rust_risk_total ~ "low"
      )
  )
```
