...
austraits <- readRDS("ignore/austraits/austraits_20250402.rds")
...
life_history_scores <- read_csv("data_extras/life_history.csv")
dispersers_scores <- read_csv("data_extras/dispersers.csv")
fruit_fleshiness_scores <- read_csv("data_extras/fruit_fleshiness_scores.csv")
resprouting_capacity_scores <- read_csv("data_extras/resprouting_capacity.csv")
...
phytophthora_cinnamomi_susceptibility_scores %>% filter(!str_detect(value, " ")) %>% arrange(phytophthora_cinnamomi_susceptibility) %>% rename(`categorical value` = value, `numeric score assigned` = phytophthora_cinnamomi_susceptibility)  %>% kable(caption = "phytophthora cinnamomi susceptibility values") %>% kable_classic()

myrtle_rust_susceptibility_scores %>% filter(!str_detect(value, " ")) %>% arrange(myrtle_rust_susceptibility) %>% rename(`categorical value` = value, `numeric score assigned` = myrtle_rust_susceptibility)  %>% kable(caption = "myrtle rust susceptibility values") %>% kable_classic()
...
```{r, echo=FALSE, results='hide', message=FALSE, warning=FALSE}
extract_categorical_trait <- function(database = all_traits, categorical_trait, trait_scores, output_name) {
  
  extracted_data <- (all_traits %>%
  austraits::extract_trait(categorical_trait))$traits %>%
  tidyr::separate_longer_delim(value, delim = " ") %>%
  left_join(trait_scores %>% rename(output_name = 3), by = join_by(trait_name, value)) %>%
  filter(!is.na(output_name)) %>%
  select(dataset_id, observation_id, taxon_name, output_name) %>%
  group_by(taxon_name, dataset_id, observation_id) %>%
  mutate(output_name = mean(as.numeric(output_name))) %>%
  ungroup() %>%
  distinct() %>%
  select(taxon_name, output_name) %>%
  group_by(taxon_name) %>%
  mutate(output_name = mean(as.numeric(output_name))) %>%
  ungroup() %>%
  distinct() %>%
...
life_history <- extract_categorical_trait(all_traits, "life_history", life_history_scores, "life_history")
...
plant_growth_form <- extract_categorical_trait(all_traits, "plant_growth_form", plant_growth_form_scores, "plant_growth_form")
...
extract_numeric_trait <- function(numeric_trait) {
  ...
  austraits::extract_data(table = "traits", col = "basis_of_record", col_value = c("field", "literature", "field literature", "literature field"), partial_matches_allowed = F) %>%
...
numeric_traits <- c("seed_dry_mass", "leaf_mass_per_area", "plant_height", "leaf_length", "leaf_mass_per_area", "leaf_N_per_dry_mass", "leaf_P_per_dry_mass", "reproductive_maturity", "seedbank_longevity", "leaf_water_content_per_area", "leaf_tannin_per_dry_mass", "leaf_phenol_per_dry_mass", "leaf_lignin_per_dry_mass", "digestible_mass_per_dry_mass", "leaf_crude_protein_per_dry_mass")

traits2 <- c("metabolisable_energy", "leaf_soluble_protein_per_area", "leaf_insoluble_protein_per_area")
...
for (var in c("life_history", "dispersers", "nitrogen_fixing", "resprouting_capacity", "fruit_fleshiness", "leaf_glaucousness", "leaf_hairs_adult_leaves", "soil_score", "geologic_substrate_score")) {
...
for (var in c("resprouting_capacity", "fruit_fleshiness", "leaf_glaucousness", "leaf_hairs_adult_leaves")) {
...
-   The three strongest correlations below don't make sense, in part due to low sample sizes:
    ...
    2.  Succulence (`leaf_water_content_per_area` ) should be positively correlated with palatability, but only scored for 4 species and happens to come out as a strong negative correlation (p-value not significant)
...
for (var in c("life_history", "nitrogen_fixing", "resprouting_capacity", "leaf_glaucousness", "leaf_hairs_adult_leaves", "plant_spinescence", "plant_growth_form")) {
...
for (var in c("leaf_mass_per_area", "plant_height", "leaf_length", "leaf_N_per_dry_mass", "leaf_P_per_dry_mass", "leaf_water_content_per_area", "leaf_lignin_per_dry_mass", "leaf_phenol_per_dry_mass", "leaf_tannin_per_dry_mass", "digestible_mass_per_dry_mass", "leaf_crude_protein_per_dry_mass")) {
