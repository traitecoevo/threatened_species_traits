...
numeric_traits <- c("seed_dry_mass", "leaf_mass_per_area", "plant_height", "leaf_length", "leaf_mass_per_area", "leaf_N_per_dry_mass", "leaf_P_per_dry_mass", "reproductive_maturity", "seedbank_longevity", "leaf_water_content_per_area", "leaf_tannin_per_dry_mass", "leaf_phenol_per_dry_mass", "leaf_lignin_per_dry_mass", "digestible_mass_per_dry_mass", "leaf_crude_protein_per_dry_mass", "leaf_work_to_shear_adjusted", "fruit_length")
...
phytophthora_assigned_risk <- phytophthora_assigned_risk %>%
  ...
  left_join(geologic_substrate %>% rename(substrate_risk = geologic_substrate), by = "taxon_name") %>%
...
deer_LMA <- deer_assigned_risk %>% filter(!is.na(leaf_mass_per_area)) %>% 
  summarise(max_leaf_mass_per_area = max(leaf_mass_per_area), min_leaf_mass_per_area = min(leaf_mass_per_area))

deer_height <- deer_assigned_risk %>% filter(!is.na(plant_height)) %>% 
  summarise(max_plant_height = max(plant_height), min_plant_height = min(plant_height))
...
deer_assigned_risk <- deer_assigned_risk %>%
  mutate(
      ...
      height_risk_nudge = -0.1*(1 - (log10(plant_height)/log10(deer_height$max_plant_height[1]))),
      height_risk_nudge = ifelse(!is.na(height_risk_nudge) & height_risk_nudge > 0, 0, height_risk_nudge),

      # add LMA nudge
      # XXX - wondering if we make max risk at an intermediate height??? - XXX
      # have chosen a risk adjustment of 0 - 0.3, because being tall (a tree) versus short (an herb/grass) would represent a shift of one unit of the threat scale
      LMA_risk_nudge = -0.1*(1 - (log10(leaf_mass_per_area)/log10(deer_LMA$max_leaf_mass_per_area[1]))),
...
pig_fruit_length <- pigs_assigned_risk %>% filter(!is.na(fruit_length)) %>% 
  summarise(max_fruit_length = max(fruit_length), min_fruit_length = min(fruit_length))

pig_height <- pigs_assigned_risk %>% filter(!is.na(plant_height)) %>% 
  summarise(max_plant_height = max(plant_height), min_plant_height = min(plant_height))
...
pigs_assigned_risk <- pigs_assigned_risk %>%
  mutate(
      ...
      height_risk_nudge = 0.1*(1 - (log10(plant_height)/log10(pig_height$max_plant_height[1]))),
      height_risk_nudge = ifelse(!is.na(height_risk_nudge) & height_risk_nudge > 0, 0, height_risk_nudge),
      ...
      fruit_length_risk_nudge = (log10(fruit_length)/log10(pig_fruit_length$max_fruit_length[1])),
