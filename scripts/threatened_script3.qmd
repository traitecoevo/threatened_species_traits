...
storage_organ_scores %>% filter(!str_detect(trait_value, " ")) %>% arrange(storage_organ)  %>% kable(caption = "storage organ values") %>% kable_classic()
...
storage_organ_pigs <- extract_categorical_trait(all_traits, "storage_organ", storage_organ_scores %>% select(trait_name, trait_value, storage_organ = storage_organ_pigs), "storage_organ_pigs")
...
by_genus <- phytophthora_cinnamomi_susceptibility_data %>%
  ...
  filter(taxa_scored_per_genus > 5, !is.na(genus)) %>%
  arrange(-phytophthora_cinnamomi_susceptibility)
...
# for continuous traits need to know min/max value for scaling
max_height <- max(plant_height$plant_height, na.rm = T)
max_leaf_mass_per_area <- max(leaf_mass_per_area$leaf_mass_per_area, na.rm = T)
max_leaf_P_per_dry_mass <- max(leaf_P_per_dry_mass$leaf_P_per_dry_mass, na.rm = T)
max_leaf_N_per_dry_mass <- max(leaf_N_per_dry_mass$leaf_N_per_dry_mass, na.rm = T)

min_height <- min(plant_height$plant_height, na.rm = T)
min_leaf_mass_per_area <- min(leaf_mass_per_area$leaf_mass_per_area, na.rm = T)
min_leaf_P_per_dry_mass <- min(leaf_P_per_dry_mass$leaf_P_per_dry_mass, na.rm = T)
min_leaf_N_per_dry_mass <- min(leaf_N_per_dry_mass$leaf_N_per_dry_mass, na.rm = T)
...
phytophthora_assigned_risk <- phytophthora_assigned_risk %>%
  mutate(
    # default habitat_risk is set to is 0.5
      habitat_risk = ifelse(is.na(habitat_phytophthora), .5, habitat_phytophthora),
    
    # high leaf P lowers susceptibility
      leaf_P_risk_nudge = (log10(leaf_P_per_dry_mass)/log10(max_leaf_P_per_dry_mass))-
        (log10(min_leaf_P_per_dry_mass)/log10(max_leaf_P_per_dry_mass)),
      leaf_P_risk_nudge = ifelse(!is.na(leaf_P_risk_nudge) & leaf_P_risk_nudge < 0, 0, -0.1*leaf_P_risk_nudge),
    
    # high leaf N lowers susceptibility
      leaf_N_risk_nudge = (log10(leaf_N_per_dry_mass)/log10(max_leaf_N_per_dry_mass))-
        (log10(min_leaf_N_per_dry_mass)/log10(max_leaf_N_per_dry_mass)),
      leaf_N_risk_nudge = ifelse(!is.na(leaf_N_risk_nudge) & leaf_N_risk_nudge < 0, 0, -0.1*leaf_N_risk_nudge),
    
    # resprouting species are more likely to regenerate, persist; 0--0.1 nudge
      resprouting_risk_nudge = -0.1*resprouting_capacity,
    
    # familes and genera with particularly high or low susceptibilities get a taxonomic nudge
      ...
      taxonomic_risk_nudge = ifelse((family %in% families_for_taxonomic_nudges$family | genus %in% genera_for_taxonomic_nudges$genus) &
        taxon_average_score < 1, -0.5, 0),
      taxonomic_risk_nudge = ifelse((family %in% families_for_taxonomic_nudges$family | genus %in% genera_for_taxonomic_nudges$genus) &
        taxon_average_score > 1, 0.8, taxonomic_risk_nudge),
    
    # sum together trait and taxonomic nudges
      total_trait_nudge = rowSums(across(c(taxonomic_risk_nudge, leaf_P_risk_nudge, leaf_N_risk_nudge, resprouting_risk_nudge)), na.rm = TRUE),
      
    # sum together habitat risk and trait/taxonomic nudge
    # rescale trait risk to be between 0.4-1.3; this value then to be multiplied by habitat risk
    # decided on mean and range by looking at distribution of values  
      total_trait_nudge_scaled = scales::rescale(total_trait_nudge, to = c(0.3, 1.2)),
      #total_trait_nudge_final = total_trait_nudge_scaled - mean(total_trait_nudge_scaled) + 0.5,
  ...
  phytophthora_assigned_risk <- phytophthora_assigned_risk %>%
    mutate(
      #phytophthora_risk_total = ifelse(high_extinction_risk == "high_extinction_risk", 1, phytophthora_risk_total),
...
myrtle_rust_assigned_risk2 <- myrtle_rust_assigned_risk %>%
  mutate(
    ...
    habitat_risk = ifelse(!is.na(habitat_myrtle_rust), habitat_myrtle_rust, 0.8),     
    # update value to incorporate habitat_moisture info; take the "wetter" of two values
    #habitat_risk = ifelse(!is.na(habitat_moisture) & !is.na(habitat_myrtle_rust) & habitat_moisture > habitat_myrtle_rust, habitat_moisture, habitat_risk),
    ...
    filter(!is.na(threatened_status)) %>%
  select(taxon_name, family, habitat_risk, trait_risk_nudge, taxonomic_risk, myrtle_rust_risk_total, myrtle_rust_final_susceptibility_score, myrtle_rust_susceptibility, everything()) %>%
...
#### - goat specific attrbitutes, scoring values
...
goats_assigned_risk <- goats_assigned_risk %>%
  mutate( 
      ...
      # add plant height nudge; 
      # maximum height among species in training dataset is 32.5 m, so setting that as the cut-off; all taller species assigned same risk adjustment as a 30 m tall species
      # have chosen a nudge of 0 - 0.3, because being tall (a tree) versus short (an herb/grass) would represent a shift of one unit of the threat scale
      height_risk_nudge = 0.1*(1 - (log10(plant_height)/log10(32.5))),
      ...
      resprouting_risk_nudge = -0.05*(1 - (resprouting_capacity/1)),
      
      # add taxonomic nudges
      Chenopodiaceae_risk_nudge = ifelse(family == "Chenopodiaceae", -0.2, 0),
      ...
      # sum together trait and taxonomic nudges
      total_trait_nudge = rowSums(across(c(height_risk_nudge, resprouting_risk_nudge, Chenopodiaceae_risk_nudge, graminoid_risk_nudge, herb_risk_nudge)), na.rm = TRUE),
      
      # sum together habitat risk and trait/taxonomic nudge
      goat_risk_total = rowSums(across(c(habitat_risk, total_trait_nudge)), na.rm = TRUE),
...
# for continuous traits need to know max value for scaling
max_height <- max(plant_height$plant_height, na.rm = T)
max_leaf_mass_per_area <- max(leaf_mass_per_area$leaf_mass_per_area, na.rm = T)
...
deer_assigned_risk <- deer_assigned_risk %>%
  mutate(
      ...
      # add plant height nudge
      # XXX - wondering if we make max risk at an intermediate height??? - XXX
      ...
      height_risk_nudge = -0.1*(1 - (log10(plant_height)/log10(max_height))),
      height_risk_nudge = ifelse(!is.na(height_risk_nudge) & height_risk_nudge > 0, 0, height_risk_nudge),

      # add LMA nudge
      # XXX - wondering if we make max risk at an intermediate height??? - XXX
      # have chosen a risk adjustment of 0 - 0.3, because being tall (a tree) versus short (an herb/grass) would represent a shift of one unit of the threat scale
      LMA_risk_nudge = -0.1*(1 - (log10(leaf_mass_per_area)/log10(max_leaf_mass_per_area))),
      LMA_risk_nudge = ifelse(!is.na(LMA_risk_nudge) & LMA_risk_nudge > 0, 0, LMA_risk_nudge),
      ...
      woodiness_risk_nudge = -0.2*woodiness,
      
      # woody species less impacted
      defense_nudge = -0.2*plant_physical_defence_structures,
      
      # sum together trait and taxonomic nudges
      total_trait_nudge = rowSums(across(c(height_risk_nudge, resprouting_risk_nudge, woodiness_risk_nudge, LMA_risk_nudge, defense_nudge)), na.rm = TRUE),
      
      # sum together habitat risk and trait/taxonomic nudge
      # rescale trait risk to be between 0.4-1.3; this value then to be multiplied by habitat risk
      # decided on mean and range by looking at distribution of values  
      total_trait_nudge_scaled = scales::rescale(total_trait_nudge, to = c(0.3, 1.2)),
      total_trait_nudge_final = total_trait_nudge_scaled - mean(total_trait_nudge_scaled) + 0.75,

      # multiple scaled trait nudge risk by habitat risk
      deer_risk_total = habitat_risk*total_trait_nudge_final
      
      # sum together habitat risk and trait/taxonomic nudge
      #deer_risk_total = rowSums(across(c(habitat_risk, total_trait_nudge)), na.rm = TRUE)
...
```
# TO DISCUSS:
# 1. There are currently too few species with moderate to high risk, because trait nudges here just reduce susceptibility
# 2. There is also the problem that lack of data is a "0" which for each trait nudge is "maximum trait risk". Should I turn all blanks into the mean nudge for that trait; also an issue for other threats
# 3. Certainty of score? Only certainty scoring would be that species with known risk = certain; others = moderate uncertainty; not knowning habitat gives low level of certainty
```
# Pigs
...
pigs_assigned_risk <- resources$APC %>% 
  ...
  left_join(resprouting_capacity, by = "taxon_name") %>%
  left_join(life_history, by = "taxon_name") %>%
  left_join(storage_organ_pigs, by = "taxon_name") %>%
...
max_fruit_length <- max(fruit_length$fruit_length, na.rm = T)
min_fruit_length <- min(fruit_length$fruit_length, na.rm = T)
...
pigs_assigned_risk <- pigs_assigned_risk %>%
  mutate(
      ...
      # add plant height nudge
      # shorter plants more impacted by pigs; risk adjustment of 0 - 0.3
      height_risk_nudge = 0.1*(1 - (log10(plant_height)/log10(max_height))),
      height_risk_nudge = ifelse(!is.na(height_risk_nudge) & height_risk_nudge > 0, 0, height_risk_nudge),
      
      # add fruit length nudge
      # plants with bigger fruits more impacted by pigs (if fleshy); risk adjustment of 0 - 1, to be multiplied by a fleshiness value
      fruit_length_risk_nudge = (log10(fruit_length)/log10(pig_fruit_length$max_fruit_length)),
      fruit_length_risk_nudge = ifelse(!is.na(fruit_length_risk_nudge) & fruit_length_risk_nudge < 0, 0, fruit_length_risk_nudge),
      
      # species with fleshy fruits more impacted; total nudge is 0--0.2
      fruit_fleshiness_risk_nudge = 0.4*fruit_fleshiness,
      ...
      storage_organ_pigs_nudge = (0.4*storage_organ_pigs),
      ...
      # sum together trait and taxonomic nudges
      total_trait_nudge = rowSums(across(c(height_risk_nudge, resprouting_risk_nudge, storage_organ_pigs_nudge, woodiness_risk_nudge, fruit_risk_nudge, taxonomic_risk_nudge)), na.rm = TRUE),
      ...
      # sum together habitat risk and trait/taxonomic nudge
      pig_risk_total = habitat_risk*total_trait_nudge_final
  )
...
max_reproductive_maturity <- max(reproductive_maturity$reproductive_maturity, na.rm = T)
min_reproductive_maturity <- min(reproductive_maturity$reproductive_maturity, na.rm = T)
...
tropical_grasses_assigned_risk_final <- tropical_grasses_assigned_risk %>%
  mutate(
    ...
    tropical_grasses_final_susceptibility_score = case_when(
        ...
        tropical_grasses_persistance_risk_total_scaled < 0.15 ~ "1_no_risk"
      )
  ) %>%
    filter(!is.na(threatened_status))
