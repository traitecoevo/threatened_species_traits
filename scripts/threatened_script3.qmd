...
life_history_scores %>% arrange(life_history) %>% rename(`categorical value` = trait_value, `numeric score assigned` = life_history)  %>% kable(caption = "life history values") %>% kable_classic()
...
### Habitat scorings for Hymenachne
...
hymenachne_assigned_risk <- hymenachne_assigned_risk %>%
  mutate(
    # woody taxa = lower impact
      plant_growth_substrate_nudge = calculate_nudge("plant_growth_substrate", 1, .),
    ...
    # if no value for habitat, use habitat moisture value
      habitat_risk = ifelse(!is.na(plant_growth_substrate_nudge), 
                            plant_growth_substrate_nudge,
                            habitat_risk),
    ...
    # annuals, ephemerals have highest impact
      life_history_risk_nudge = calculate_nudge("life_history", -0.3, .),

    # taxonomy nudge
    # add taxonomic nudges

      taxonomic_risk_nudge = ifelse(family %in% c("Arecaceae", "Cyperaceae", "Haloragaceae", "Hydrocharitaceae", "Iridaceae", 
                                                  "Juncaceae", "Marsileaceae", "Myrtaceae", "Nymphaeaceae", "Orchidaceae", 
                                                  "Poaceae", "Pontederiaceae", "Restionaceae", "Typhaceae"),
                                    0.3, -0.3),
    
    # sum trait risk nudges
      total_trait_nudge = rowSums(across(c(height_risk_nudge, life_history_risk_nudge, woodiness_risk_nudge, taxonomic_risk_nudge)), na.rm = TRUE),
    

    # rescale trait risk; this value then to be multiplied by habitat risk
    # decide on range by looking at distribution of values and deciding, for a given risk, how much traits are "allowed" to offset habitat risk  
      total_trait_nudge_scaled = scales::rescale(total_trait_nudge, to = c(0.6, 1.2)),

    # multiple scaled trait nudge risk by habitat risk
      hymenachne_risk_total = habitat_risk*total_trait_nudge_scaled
  )

# assign risk by category
  hymenachne_assigned_risk <- hymenachne_assigned_risk %>%
    mutate(
      hymenachne_final_susceptibility_score = case_when(
          hymenachne_risk_total >= 0.8 ~ "5_extremely_high",
          hymenachne_risk_total >= 0.6 & hymenachne_risk_total < 0.8 ~ "4_high",
          hymenachne_risk_total >= 0.4 & hymenachne_risk_total < 0.6 ~ "3_moderate",
          hymenachne_risk_total >= 0.2 & hymenachne_risk_total < 0.4 ~ "2_low",
          hymenachne_risk_total < 0.2 ~ "1_no_risk"
        )
    ) %>%
      filter(!is.na(threatened_status)) %>%
    write_csv("export/hymenachne_assigned_risk_final.csv")
```
