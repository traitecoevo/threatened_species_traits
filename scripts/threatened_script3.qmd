...
numeric_traits <- c("seed_dry_mass", "leaf_mass_per_area", "plant_height", "leaf_length", "leaf_mass_per_area", "leaf_N_per_dry_mass", "leaf_P_per_dry_mass", "reproductive_maturity", "seedbank_longevity", "leaf_water_content_per_area", "leaf_tannin_per_dry_mass", "leaf_phenol_per_dry_mass", "leaf_lignin_per_dry_mass", "digestible_mass_per_dry_mass", "leaf_crude_protein_per_dry_mass", "leaf_work_to_shear_adjusted")
...
tropical_grasses_assigned_risk <- tropical_grasses_assigned_risk %>%
  mutate(
    ...
    habitat_risk2 = ifelse(!is.na(habitat) & 
                                    (str_detect(habitat, "savanna") | 
                                     ...
                                     str_detect(habitat, "agriculture") | 
                                     str_detect(habitat, "roadside")), 
                                      1, habitat_risk),
    habitat_risk2 = ifelse(!is.na(habitat_risk2) & 
                                    (str_detect(habitat, "alpine") | 
    ...
    # geophytes, other plants with belowground storage organ = negative, less impact
    storage_organ_risk_nudge = ifelse(geophyte > 0 | storage_organ > 0, -0.4, 0),
    # annuals = 0, perennials = positive, more impact
    life_history_risk_nudge = ifelse(!is.na(life_history), 0.15*life_history, 0),
    # woody taxa = positive, more impact
    woodiness_risk_nudge = ifelse(!is.na(woodiness), 0.4*woodiness, 0),
    # resprouters = negative, less impact
    ...
    direct_competition_risk = ifelse(!is.na(plant_height) | (!is.na(dry_season_flowering) & life_history < 1.5), -0.3*dry_season_flowering + plant_height_risk_nudge + plant_height_risk_nudge, NA),
    ...
    # most impacted by increased fire intensity if you are woody and with high bud bank location 
    fire_intensity_risk = rowSums(across(c(woodiness_risk_nudge, storage_organ_risk_nudge, bud_bank_location_nudge)), na.rm = TRUE),
    ...
    tropical_grasses_persistance_risk_total = rowSums(across(c(direct_competition_risk, fire_season_risk, fire_intensity_risk, fire_frequency_risk)), na.rm = TRUE)
  )
...
tropical_grasses_assigned_risk <- tropical_grasses_assigned_risk %>%
  mutate(
    # normalise risks
    tropical_grasses_persistance_risk_total_scaled = (tropical_grasses_persistance_risk_total - (3*min_trait_risk/2)) / (range_trait_risk),
