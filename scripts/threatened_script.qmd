...
life_history <- austraits$traits %>% filter(trait_name == "life_history")  %>% 
  left_join(life_history_scores) %>%
...
dispersers <- austraits$traits %>% filter(trait_name %in% c("dispersal_syndrome", "dispersers"))  %>% 
  left_join(dispersers_scores) %>%
...
resprouting_capacity <- austraits$traits %>% filter(trait_name %in% c("resprouting_capacity"))  %>% 
  left_join(resprouting_capacity_scores) %>%
...
nitrogen_fixing <- austraits$traits %>% filter(trait_name %in% c("nitrogen_fixing"))  %>% 
  left_join(nitrogen_fixing_scores) %>%
...
fruit_fleshiness <- (austraits %>%
  austraits::extract_trait("fruit_fleshiness"))$traits %>%
  ...
  ungroup() %>%
  distinct(taxon_name, fruit_fleshiness)

(austraits %>%
  austraits::extract_trait("nitrogen_fixing", partial_matches_allowed = F))$traits %>%
  group_by(taxon_name) %>%
  distinct(value) %>% mutate(nitrogen_fixing = 1)
```

# phytophthora

```{r}
phytophthora_cinnamomi_susceptibility <- (threatened_species %>%
                                            extract_trait("phytophthora_cinnamomi_susceptibility") %>%
                                            join_taxa(vars = "family"))$traits %>%
  select(taxon_name, family, value) %>%
  group_by(taxon_name) %>%
  mutate(value = paste0(value, collapse = " ")) %>% 
  ungroup() %>%
  distinct() %>% 
  left_join(phytophthora_cinnamomi_susceptibility_scores) %>%
  left_join(nitrogen_fixing) %>% # p = 0.04, R^2 = 0.007
  left_join(dispersers) %>% #p = 0.06, R^2 = 0.007
  left_join(life_history) %>% #R^2 = 0.01, p < 0.000001, coeff = 0.003
  left_join(seed_dry_mass) %>%
  left_join(leaf_mass_per_area) %>% #R2 = 0.05, higher LMA = more severe effect, coeff = 30.63
  left_join(fruit_fleshiness) %>% #no effect - p = 0.51
  left_join(resprouting_capacity) #R^2 = 0.03, p < 0.000001, coef = 0.43; fire-killed more susceptible

phyt_lm <- lm(phytophthora_cinnamomi_susceptibility ~ resprouting_capacity, data = phytophthora_cinnamomi_susceptibility)
...
by_family <- phytophthora_cinnamomi_susceptibility %>%
  ...
  ungroup() %>%
  distinct(family, taxa_scored, phytophthora_cinnamomi_susceptibility) %>%
  filter(taxa_scored > 10, !is.na(family)) %>%
...
ggplot(data = family_histograms) +
  geom_histogram(aes(x = phytophthora_cinnamomi_susceptibility)) +
  facet_wrap(~family)
```
...
probability_0_1 <- probability_0_1 %>%
  ...
  left_join(dispersers) %>%
  left_join(life_history) %>%
  left_join(seed_dry_mass) %>%
  left_join(leaf_mass_per_area) %>%
  left_join(fruit_fleshiness) %>%
  left_join(resprouting_capacity)
```

# checking correlations

```{r}
rust_lm <- lm(resprouting_capacity ~ myrtle_rust_susceptibility_numeric , data = probability_0_1)
...
# threatened very slightly worse dispersal, but meaningly (r2 = 0.0008)
dispersers_by_threat_lm <- lm(threatened ~ dispersers_score, data = filter(all_taxa, !is.na(dispersers_score)))

# absolutely no variation in seed mass
seedmass_by_threat_lm <- lm(threatened ~ seed_dry_mass, data = filter(all_taxa, !is.na(seed_dry_mass)))
