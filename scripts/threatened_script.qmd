...
leaf_soluble_protein_per_area <- all_traits %>%
  austraits::extract_trait("leaf_soluble_protein_per_area") %>%
  austraits_weighted_means("leaf_soluble_protein_per_area") %>%
  filter(taxon_name %in% resources$`APC list (accepted)`$canonical_name) %>%
  select(taxon_name, leaf_soluble_protein_per_area = mean)
  
leaf_insoluble_protein_per_area <- all_traits %>%
  austraits::extract_trait("leaf_insoluble_protein_per_area") %>%
  austraits_weighted_means("leaf_insoluble_protein_per_area") %>%
  filter(taxon_name %in% resources$`APC list (accepted)`$canonical_name) %>%
  select(taxon_name, leaf_insoluble_protein_per_area = mean)
  
leaf_lignin_per_dry_mass <- all_traits %>%
  austraits::extract_trait("leaf_lignin_per_dry_mass") %>%
  austraits_weighted_means("leaf_lignin_per_dry_mass") %>%
  filter(taxon_name %in% resources$`APC list (accepted)`$canonical_name) %>%
  select(taxon_name, leaf_lignin_per_dry_mass = mean)

leaf_phenol_per_dry_mass <- all_traits %>%
  austraits::extract_trait("leaf_phenol_per_dry_mass") %>%
  austraits_weighted_means("leaf_phenol_per_dry_mass") %>%
  filter(taxon_name %in% resources$`APC list (accepted)`$canonical_name) %>%
  select(taxon_name, leaf_phenol_per_dry_mass = mean)

leaf_tannin_per_dry_mass <- all_traits %>%
  austraits::extract_trait("leaf_tannin_per_dry_mass") %>%
  austraits_weighted_means("leaf_tannin_per_dry_mass") %>%
  filter(taxon_name %in% resources$`APC list (accepted)`$canonical_name) %>%
  select(taxon_name, leaf_tannin_per_dry_mass = mean)
...
for (var in c("resprouting_capacity", "seed_dry_mass", "leaf_mass_per_area", "fruit_fleshiness", "plant_height", "leaf_length", "leaf_glaucousness", "leaf_hairs_adult_leaves", "leaf_N_per_dry_mass", "leaf_P_per_dry_mass", "leaf_lignin_per_dry_mass", "leaf_phenol_per_dry_mass", "leaf_tannin_per_dry_mass")) {
